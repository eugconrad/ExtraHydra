...
from typing import List, Iterable, TypeVar, Sequence, Set, Optional
...
def main(args: List[str]) -> None:
    ...
    if app_path:
        print(f'Setup app: {app_path}', file=sys.stderr, flush=True)
        setup_app(board, app_path)
...
def setup_app(board: Pyboard, app_path: Optional[str]) -> None:
    """Setup app path in RTC memory before reset."""
    try:
        board.enter_raw_repl()
        board.exec_raw_no_follow("from machine import RTC, reset")
        board.exec_raw_no_follow(f"RTC().memory(b'{app_path}')")
        # board.exec_raw_no_follow("reset()")
        board.exit_raw_repl()
    except Exception as e:
        print(f"Warning: App setup failed - {e}", file=sys.stderr)
